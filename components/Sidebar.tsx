            ...
            return (
              <div key={pillar.id} className="space-y-3">
                ...
                >
                  ...
                  <span className="text-xs font-bold truncate leading-tight flex-1">{pillar.title || 'Pilar'}</span>
                  {isPillarSelected ? <ChevronDown size={16} className="ml-auto" /> : <ChevronRight size={16} className="ml-auto" />}
                </button>

                {isPillarSelected && safeVariations.length > 0 && (
                    ...
                    {safeVariations.map(v => {
                      ...
                      return (
                        <div key={v.id} className="space-y-1">
                          ...
                          >
                            {scoreData ? (
                              <CheckCircle2 size={16} className={`${scoreData.score === scoreData.total ? 'text-emerald-500' : 'text-rose-500'} shrink-0`} />
                            ) : (
                              <Layout size={16} className={isVarSelected ? 'text-orange-500' : 'text-slate-700'} />
                            )}
                            <span className="flex-1 truncate">{v.title || 'Idea'}</span>
                          </button>

                          {isVarSelected && course && Array.isArray(course.modules) && (
                            <div className="ml-4 pl-4 border-l border-slate-800 space-y-2 mt-2 mb-4">
                              {course.modules.map((m, idx) => (
                                ...
                                >
                                  ...
                                  <span className="truncate">{m.title || 'Módulo'}</span>
                                </button>
                              ))}
                            </div>
                          )}
