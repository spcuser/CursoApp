  ...
  const handleOpenSelectKey = async () => {
    try {
      if (window.aistudio && typeof window.aistudio.openSelectKey === 'function') {
        ...
        alert("El selector de claves no está disponible en este entorno.");
      }
    } catch (e) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden transition-colors border border-slate-200 dark:border-slate-700">
        ...
        <div className="p-6 space-y-6">
          ...
          {/* API KEY SECTION - IMPORTANT FOR QUOTA ISSUES */}
          <div className="p-5 bg-indigo-50 dark:bg-indigo-900/20 rounded-2xl border border-indigo-100 dark:border-indigo-800/30 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <h4 className="font-bold text-slate-900 dark:text-white">Uso de API y Límites</h4>
            </div>
            <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
              Si experimentas errores de "cuota excedida" al generar eBooks, te recomendamos usar tu propia clave API de un proyecto con facturación activa.
            </p>
            <button 
              onClick={handleOpenSelectKey}
              className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-xs uppercase tracking-wider shadow-lg shadow-indigo-500/20 transition-all"
            >
              Configurar Mi Clave API
            </button>
            <a 
              href="https://ai.google.dev/gemini-api/docs/billing" 
              target="_blank" 
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-1 text-[10px] text-indigo-500 hover:underline font-bold uppercase tracking-tighter"
            >
          ...
          <div className="pt-4 border-t border-slate-100 dark:border-slate-700">
             ...
             <button 
               ...
               className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600 rounded-xl transition-colors text-xs font-bold uppercase tracking-wide shadow-sm"
             >
               <Download size={18} />
